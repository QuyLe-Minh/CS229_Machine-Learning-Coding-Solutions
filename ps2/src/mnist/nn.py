...
def forward_prop(data, labels, params):
    ...
    loss = -np.sum(labels * np.log(output)) * 1/batch_size
...
def backward_prop_regularized(data, labels, params, forward_prop_func, reg):
    ...
    a1, a2, loss = forward_prop_func(data, labels, params)
    dz2 = a2 - labels
    dW2 = 1/m * (a1.T @ dz2) + reg * 2 * W2
    db2 = 1/m * np.sum(dz2, axis = 0, keepdims=True)
    dz1 = (dz2 @ W2.T) * a1 * (1-a1)
    dW1 = 1/m * (data.T @ dz1) + reg * 2 * W1
    db1 = 1/m * np.sum(dz1, axis = 0, keepdims=True)
    grads = {"dW2":dW2, "db2":db2, "dW1":dW1, "db1":db1}
    return grads
    # *** END CODE HERE ***
...
def gradient_descent_epoch(train_data, train_labels, learning_rate, batch_size, params, forward_prop_func, backward_prop_func):
    ...
    # *** START CODE HERE ***
    a1, a2, loss = forward_prop_func(train_data, train_labels, params)
    grads = backward_prop_func(train_data, train_labels, params, forward_prop_func)
    for i in range(1, train_data.shape[0] // batch_size):
        grads = backward_prop_func(train_data[(i-1)*batch_size : i*batch_size, :], train_labels[(i-1)*batch_size : i*batch_size, :], params, forward_prop_func)
...
def main(plot=True):
    ...
    np.random.seed(100)
    train_data, train_labels = read_data('D:\Coding\ps2\src\mnist\images_train.csv', 'D:\Coding\ps2\src\mnist\labels_train.csv')
    ...
    test_data, test_labels = read_data('D:\Coding\ps2\src\mnist\images_test.csv', 'D:\Coding\ps2\src\mnist\labels_test.csv')
