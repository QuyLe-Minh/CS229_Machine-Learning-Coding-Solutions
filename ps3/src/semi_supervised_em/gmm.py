...
def main(is_semi_supervised, trial_num):
    ...
    for i in range(K):
        ...
        sigma[i] = ((sample - mu[i]).T @ (sample - mu[i])) / len(sample)
...
def run_em(x, phi, mu, sigma):
    ...
    while it < max_iter and (prev_ll is None or np.abs(ll - prev_ll) >= eps):
        ...
        for j in range(K):
            ...
            log_prob[:,j] = - np.log(np.linalg.det(sigma[j])) / 2 - exponents / 2 + np.log(phi[j])
        ...
        prev_ll = ll
        ll = np.log(np.exp(log_prob).sum(axis = 1)).sum()  - n * dim / 2 * np.log(2 * np.pi)
        it+=1
        # *** END CODE HERE ***
...
def run_semi_supervised_em(x, x_tilde, z_tilde, phi, mu, sigma):
    ...
    while it < max_iter and (prev_ll is None or np.abs(ll - prev_ll) >= eps):
        ...
        prev_ll = ll
        ll = np.log(np.exp(log_prob[:n]).sum(axis = 1)).sum() + (w_tilde * log_prob[n:]).sum() - (n+n_tilde) * dim / 2 * np.log(2 * np.pi) 
        # Hint: For debugging, recall part (a). We showed that ll should be monotonically increasing.
        it+=1
        # *** END CODE HERE ***
    print("Converge after {} iterations with loss: {}".format(it, ll))
