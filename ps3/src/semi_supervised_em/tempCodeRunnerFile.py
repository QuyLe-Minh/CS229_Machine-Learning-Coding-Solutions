...
def main(is_semi_supervised, trial_num):
    ...
    # Load dataset
    train_path = os.path.join('.', 'C:/Users/Admin/OneDrive - hcmut.edu.vn/Giáo trình/Machine Learning/Coding/ps3/src/semi_supervised_em/train.csv')
    ...
    mu = np.empty((K, dim))
    permutations = np.random.randint(K, size = n)
    for i in range(K):
        sample = x_all[permutations == i]
        mu[i] = np.mean(sample, axis = 0)
        sigma[i] = ((sample - mu[i]).T @ (sample - mu[i])) / n
...
def run_em(x, phi, mu, sigma):
    ...
    while it < max_iter and (prev_ll is None or np.abs(ll - prev_ll) >= eps):
        ...
        for j in range(K):
            exponents = (x - mu[j]) @ np.linalg.inv(sigma[j]) * (x - mu[j])
            exponents = exponents.sum(axis=1)
            log_prob[:,j] = - np.log(np.linalg.det(sigma[j])) / 2 - exponents / 2 + np.log(phi[j]) - np.log(2*np.pi) * dim/2
            
        w = log_prob - np.log(np.exp(log_prob).sum(axis = 1, keepdims=True))
        w = np.exp(w)
        ...
        prev_ll = ll
        ll = np.log(np.exp(log_prob).sum(axis = 1)).sum()
        it+=1
        # *** END CODE HERE ***
    print("Converge after {} iterations with loss: {}".format(it, ll))
    return w
...
def run_semi_supervised_em(x, x_tilde, z_tilde, phi, mu, sigma):
    ...
    n, dim = x.shape
    n_tilde, dim_tilde = x_tilde.shape
    w_tilde = np.empty((n_tilde, K))
    log_prob = np.empty((n + n_tilde, K))
    w_tilde = alpha * (z_tilde == np.arange(K))
    x = np.vstack([x, x_tilde])
    ...
    while it < max_iter and (prev_ll is None or np.abs(ll - prev_ll) >= eps):
        ...
        # (1) E-step: Update your estimates in w
        for j in range(K):
            e =  (x-mu[j]) @ np.linalg.inv(sigma[j]) * (x-mu[j])
            e = np.sum(e, axis = 1)
            log_prob[:, j] = -e/2 - np.log(2*np.pi)*dim/2 - np.log(np.linalg.det(sigma[j])) * 1/2 + np.log(phi[j])
        ...
        # (2) M-step: Update the model parameters phi, mu, and sigma
        w[n:] = w_tilde
        phi = np.sum(w, axis = 0) / (n + n_tilde * alpha)
        for j in range(K):
            mu[j] = (w[:, j] @ x)/ np.sum(w[:, j])
            sigma[j] = w[:, j] * (x - mu[j]).T @ (x-mu[j])/np.sum(w[:, j])
        # (3) Compute the log-likelihood of the data to check for convergence.
        ...
        prev_ll = ll
        ll = np.log(np.exp(log_prob[:n]).sum(axis = 1)).sum() + (w_tilde * log_prob[n:]).sum() 
        # Hint: For debugging, recall part (a). We showed that ll should be monotonically increasing.
        it+=1
        # *** END CODE HERE ***
    print("Converge after {} iterations with loss: {}".format(it, ll))
