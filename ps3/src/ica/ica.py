...
def update_W(W, x, learning_rate):
    ...
    # *** START CODE HERE ***
    vector = W @ x
    vector = np.sign(vector).reshape(-1, 1)
    updated_W = W + learning_rate*(np.linalg.inv(W).T - vector * x)
    # *** END CODE HERE ***
...
def load_data():
    mix = np.loadtxt('C:/Users/Admin/OneDrive - hcmut.edu.vn/Giáo trình/Machine Learning/Coding/ps3/src/ica/mix.dat')
    return mix
